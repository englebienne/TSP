...
generate = 0;
/* genside = -1; // Generate all sides by default */
...
)
{
    ...
    { 
        union() 
        {
            ...
            {
                 simpleTab(simple_tabs[t][0], simple_tabs[t][1], simple_tabs[t][2], simple_tabs[t][3], thickness);
            }
        ...
        {
            simpleTabHole(simple_tab_holes[t][0], simple_tab_holes[t][1], simple_tab_holes[t][2], simple_tabs[t][3], thickness);
        }
        if(captive_nuts != undef) for (t = [0:1:len(captive_nuts)-1]) 
        {
...
module simpleTab(angle, x, y, width, thickness)
{
     let (w = width != undef ? width : thickness) {
          translate([x,y,0]) rotate([0,0,angle]) translate([-w/2,0,0]) cube([w, thickness, thickness]);
     }
}

module simpleTabHole(angle, x, y, width, thickness)
{
     // Special case does not go past edge - so make only 1 thickness y
     if (angle == 360)
     {
         translate([x,y,0]) rotate([0,0,0]) translate([0,0,-thickness]) cube([thickness, thickness, thickness*3]); /* not sure how this works */
     }
     else
     {
         translate([x,y,0]) rotate([0,0,angle-180]) translate([-width/2,-thickness,-thickness]) cube([width, thickness, thickness*3]); 
     }
}
...
)
{
    if (sides==3)
    {
        ...
        translate([0,thickness,0]) lasercutoutBoxAdjustedFJ(thickness = thickness, x=x, y=y-thickness*2 , z=z-thickness, sides=sides, fj=fj,
            ...
            slits_a=slits_a,
            cutouts_a=cutouts_a,
                                                            milling_bit = milling_bit,genside=genside
        );
    }
